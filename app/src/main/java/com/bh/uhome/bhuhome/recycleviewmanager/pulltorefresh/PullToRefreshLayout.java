package com.bh.uhome.bhuhome.recycleviewmanager.pulltorefresh;
...
public class PullToRefreshLayout extends RelativeLayout {
    ...
    //过期时间
    public static final int DELAYED_TIME = 9000;//9秒
    ...
    public void refreshFinish(int refreshResult) {
        ...
        switch (refreshResult) {
            ...
            default:
                ...
                if (null == customRefreshView) {
                    refreshStateImageView.setVisibility(View.VISIBLE);
                    refreshStateTextView.setText("请求网络时间过长");
                    refreshStateImageView
                            .setBackgroundResource(R.mipmap.refresh_failed);
                }
                break;
        ...
        if (pullDownY > 0) {
            // 刷新结果停留1.5秒
            new RemainHandler(this).sendEmptyMessageDelayed(0, 1500);
        } else {
            changeState(DONE);
            hide();
        }
    ...
    public void loadmoreFinish(int refreshResult) {
        ...
        switch (refreshResult) {
            ...
            default:
                ...
                if (null == customLoadmoreView) {
                    loadStateImageView.setVisibility(View.VISIBLE);
                    loadStateTextView.setText("网络异常");
                    loadStateImageView
                            .setBackgroundResource(R.mipmap.refresh_failed);
                }
                break;
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        switch (ev.getActionMasked()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mEvents == 0) {
                    if (5 * (Math.abs(ev.getX() - lastX)) > (ev.getY() - lastY)) {
                        mCanPullDown = false;
                    } else {
                        releasePull();
                    }

                    Log.d("pull", "MotionEvent.ACTION_MOVE");
                    ...
                    if (pullDownY > 0
                        ...
                        // 可以下拉，正在加载时不能下拉
                        // 对实际滑动距离做缩小，造成用力拉的感觉
                        float x = Math.abs(ev.getX() - lastX); //x轴滑动的距离
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (state == RELEASE_TO_REFRESH) {
                    ...
                    new FreshHandler(this).sendEmptyMessageDelayed(100, DELAYED_TIME);//10秒后如果不完成自动完成
                } else if (state == RELEASE_TO_LOAD) {
                    changeState(LOADING);
                    // 加载操作
                    if (mListener != null)
                        mListener.onLoadMore(this);
                    new FreshHandler(this).sendEmptyMessageDelayed(101, DELAYED_TIME);//10秒后如果不完成自动完成
                }
                hide();
            default:
                break;
     ...
     * 刷新后启动的计时的handler
     */
    static class FreshHandler extends Handler {
        private WeakReference<PullToRefreshLayout> mLayout;

        public FreshHandler(PullToRefreshLayout layout) {
            mLayout = new WeakReference<PullToRefreshLayout>(layout);
        }
        ...
        public void handleMessage(Message msg) {
            ...
            if (null != layout) {
                switch (msg.what) {
                    case 100:
                        layout.refreshFinish(ABNORMAL);
                        this.removeMessages(100);
                        break;
                    case 101:
                        layout.loadmoreFinish(ABNORMAL);
